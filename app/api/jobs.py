...
from app.schemas.job import JobCreate, ParsedJobDescription
...
@router.post("/parse-job-image", status_code=200, response_model=ParsedJobDescription)
async def parse_job_image(
    ...
    # Send to Gemini
    ai = GeminiInference()
    image_part = types.Part.from_bytes(data=image_bytes, mime_type=file.content_type or "image/jpeg")
    
    prompt = "Extract the job posting details from this image. Return a JSON with three fields: 'company' (the hiring company name), 'role' (the job title), and 'description' (the full text of the job description and requirements)."
    
    try:
        response = await ai._call_api(
            model=ai.model,
            config_params={
                "response_mime_type": "application/json",
            },
            inputs=[prompt, image_part],
            user_id=current_user.id,
            purpose="parse_job_image"
        )
        parsed = ai.parse_output(response.text, ParsedJobDescription)
        return parsed
    except Exception as e:
        logger.error(f"Failed to parse job image: {e}")
        from fastapi import HTTPException
        raise HTTPException(status_code=500, detail="Failed to parse image with AI.")
