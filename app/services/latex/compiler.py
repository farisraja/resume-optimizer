...
async def _run_pdflatex(tmp_path: Path, env: dict, pass_num: int, pdflatex_exe: str = "pdflatex") -> tuple[int, bytes, bytes]:
...
async def compile_latex(latex_code: str) -> bytes:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        pdflatex_exe = shutil.which("pdflatex", path=env.get("PATH")) or "pdflatex"
        ...
        for pass_num in range(1, PDFLATEX_PASSES + 1):
            returncode, stdout, stderr = await _run_pdflatex(tmp_path, env, pass_num, pdflatex_exe)
