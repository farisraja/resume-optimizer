          ...
          <div class="max-w-3xl mx-auto space-y-6">
            ...
            <div class="widget-card fade-slide-in">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <div class="md:col-span-2" v-if="info.summary"><label class="kpi-label block mb-1">Professional Summary / About Me</label><p class="text-white whitespace-pre-wrap">{{ info.summary }}</p></div>
              </div>
            </div>
          ...
          <div v-if="editing && ed" class="space-y-6">
            ...
            <div class="widget-card">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <div class="md:col-span-2"><label class="kpi-label block mb-1">Professional Summary / About Me</label><textarea v-model="ed.summary" class="input-field" rows="3" placeholder="A brief professional summary or about me..."></textarea></div>
              </div>
            </div>
            <!-- Links -->
            <div class="widget-card">
...
const JobCreatePage = {
          ...
          <template v-if="!processing && !finalStatus">
            ...
            <div class="widget-card mb-6 fade-slide-in stagger-2">
              <div class="flex justify-between items-center mb-4">
                <div class="section-label mb-0">Step 2: Job Details</div>
                <div class="relative overflow-hidden inline-block">
                  <button class="btn-secondary" style="padding: 6px 12px; font-size: 11px; display: flex; align-items: center;" :disabled="parsingImage">
                    <span v-if="parsingImage" class="spinner" style="width:12px;height:12px;border-width:2px;margin-right:8px;"></span>
                    <i v-else class="fas fa-camera" style="margin-right:8px;"></i>
                    {{ parsingImage ? 'PARSING IMAGE...' : 'PARSE FROM IMAGE' }}
                  </button>
                  <input type="file" @change="handleJobImageUpload" accept="image/*" :disabled="parsingImage" class="absolute left-0 top-0 opacity-0 cursor-pointer w-full h-full" />
                </div>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                <div>
  ...
  setup() {
    ...
    const handleJobImageUpload = async (event) => {
      ...
      try {
        const data = await api.upload('/jobs/parse-job-image', file);
        ...
        error.value = 'Failed to parse image: ' + e.message;
      } finally {
    ...
    return { profileStore, selectedProfile, company, role, jobDescription, submitting, error, readyProfiles, formatDate, submit, processing, finalStatus, progress, statusLines, phaseLabel, jdLines, visibleJdLines, jdDone, retryGeneration, goToJob, reset, parsingImage, handleJobImageUpload };
  },
};
